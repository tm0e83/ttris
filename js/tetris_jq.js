(function($, window) {
	$.Tetris = function(element, options) {
		...
		this.createPreview = function() {
			...
			$('#' + self.options.preview).css({
				'width': self.options.size * 4 + 'px'
			});

			for(var row = 1; row <= 4; row++) {
				for(var column = 1; column <= 4; column++) {
					$('#' + self.options.preview).append('<div class="block row_' + row + ' col_' + column + '"></div>'); 
				}
			}
		}
		...
		this.buildBoard = function() {
			...
			$('#' + self.options.wrapper).append('<div id="' + self.options.board + '" style="width:200px;height:400px"></div>'); 
			$('#' + self.options.wrapper).append('<div id="' + self.options.gameOver + '">' + self.options.gameOverText + '</div>');

			// board grid
			for(var row = 1; row <= self.options.rows; row++) {
				for(var column = 1; column <= self.options.cols; column++) {
					$('#' + this.options.board).append('<div class="block row_' + row + ' col_' + column + '"></div>');
				}
			}
			...
			$('#' + this.options.wrapper).append(
				'<div id="' + self.options.menu + '">' +
					...
					'<ul id="' + self.options.levelDisplayWrapper + '" class="' + self.options.clearfixClass + '">' + 
						...
						'<li id="' + self.options.levelDisplay + '"></li>' +
					'</ul>' +
			...
			// brick carrier
			$('#' + this.options.board).append('<div id="' + self.options.carrier + '" style="width:' + 4 * self.options.size + 'px; height:' + 4 * self.options.size + 'px;"></div>');
			
			// carrier grid
			for(var column = 1; column <= 4; column++) {
				for(var row = 1; row <= 4; row++) {
					$('#' + self.options.carrier).append('<div class="block row_' + row + ' col_' + column + '"></div>');
				}
			}
		}
		...
		this.resetGame = function() {
			...
			$('#' + self.options.scoreBoard).html(self.score);
			$('#' + self.options.levelDisplay).html(self.level);
			$('#' + self.options.carrier + ' .' + self.options.block).removeAttr('style');
		...
		this.gameRoutine = function() {
			...
			$('#' + self.options.carrier).css({
				'top': self.y * self.options.size + 'px',
				'left': self.x * self.options.size + 'px'
			});
		...
		this.turnBrick = function() {
			...
			if(self.currentCoordIndex <= 2) nxtCoordsIndex = self.currentCoordIndex + 1;
			...
			jQuery.each(self.currentBrick.coords[nxtCoordsIndex], function(index, coord) {
				block_y = (Math.floor(coord / 4)) + self.y;
				block_x = (Math.floor(coord % 4)) + self.x;
				blockIndex = block_y * self.options.cols + block_x;

				if($('#' + self.options.board + ' .' + self.options.block).eq(blockIndex).hasClass('fixed')) {
					blocked = true;
				}
			...
			if(blocked == false && outside == false) {
				jQuery.each(self.currentBrick.coords[self.currentCoordIndex], function(index, coord) {
					$('#' + self.options.carrier + ' .' + self.options.block).eq(coord).removeAttr('style');
				});
				
				// nächstes Koordinatenset auswählen
				if(self.currentCoordIndex <= 2) self.currentCoordIndex++
				else self.currentCoordIndex = 0;
			...
			if(self.status == 'on') {
				if(dir != 'down') {
					...
					if(self.checkNextStep()) {
						...
						$('#' + self.options.carrier).css({'left': self.x * self.options.size + 'px'});
					}
				}
				...
				if(dir == 'down') {
					...
					if(self.checkNextStep()) {
						self.y++;
						$('#' + self.options.carrier).css({'top': self.y * self.options.size + 'px'});
						self.nextStep = [];
					} else {
						clearInterval(self.moveInterval);
		...
		this.checkRows = function() {
			...
			for(var row = 0; row <= self.options.rows; row++) {
				...
				$('#' + self.options.board + '>.' + 'row_' + row).each(function(index, block) {
					if($(block).hasClass(self.options.blocked) == false) removeRow = false;
			...
			if(self.deletedRows.length > 0) {
				...
				if(self.status == 'on') self.gameRoutine();
			}
		}
